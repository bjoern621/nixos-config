...
Row {
    ...
    // Multiply by 1.01 to add 1% buffer, preventing short intervals that would trigger unnecessarily often due to QML timer precision variance
    Timer {
        id: updateTimer
        interval: (60000 - (new Date().getTime() % 60000)) * 1.01
        running: true
        repeat: false
        onTriggered: {
