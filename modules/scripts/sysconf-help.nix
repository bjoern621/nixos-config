...
let
  sysconf-help = pkgs.writeShellScriptBin "sysconf-help" ''
    ...
    echo "                   Usage: sysconf-reload [<host>] [--remote]"
    echo "                   No argument: detect host from /etc/hostname"
    echo "                   --remote: build here, activate over ssh on the"
    echo "                   address the host declares as deploy.targetHost."
    echo "                   Names its host, and keeps that host's committed"
    echo "                   hardware-configuration.nix instead of copying"
