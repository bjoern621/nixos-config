...
{
  # The unprefixed secrets are the bjoern-laptop key.
  ...
  sops.secrets.garage-s3-team-access-key-id.sopsFile = ../secrets/garage.yaml;
  sops.secrets.garage-s3-team-secret-access-key.sopsFile = ../secrets/garage.yaml;
  ...
  # put the secret key in the world-readable Nix store instead. The remotes
  # themselves live in home/modules/garage-mount.nix.
  #
  ...
  sops.templates."garage-rclone.env" = {
    owner = "bjoern";
    content = ''
      RCLONE_CONFIG_LAPTOP_ACCESS_KEY_ID=${config.sops.placeholder.garage-s3-access-key-id}
      RCLONE_CONFIG_LAPTOP_SECRET_ACCESS_KEY=${config.sops.placeholder.garage-s3-secret-access-key}
      RCLONE_CONFIG_TEAM_ACCESS_KEY_ID=${config.sops.placeholder.garage-s3-team-access-key-id}
      RCLONE_CONFIG_TEAM_SECRET_ACCESS_KEY=${config.sops.placeholder.garage-s3-team-secret-access-key}
    '';
  };
}
